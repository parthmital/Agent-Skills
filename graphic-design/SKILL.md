...
|------|-----------|---------|
| Brand identity, voice, assets | `brand-guidelines` | Bundled sibling skill |
| Tokens, specs, CSS vars | `design-system` | Bundled sibling skill |
| shadcn/ui, Tailwind, code | `shadcn-tailwind-ui` | Bundled sibling skill |
...
Script paths in this skill and its `references/` are relative to the directory that contains this SKILL.md, not to the project: `scripts/<file>` is this skill's own `scripts/` folder, and `../<skill>/scripts/<file>` is a sibling sub-skill installed alongside it. Build the full path from that directory (many agent harnesses report it as the skill's base directory when the skill loads) and keep the working directory at the project root — the scripts read and write project files such as `docs/brand-guidelines.md`, `assets/design-tokens.json` or `src/` relative to it.
...
After generation, **ALWAYS** ask the user about HTML preview. If yes, use the bundled `ui-ux-design-reference` skill for the gallery.
...
2. **Research** — Read `references/banner-sizes-and-styles.md` and use the bundled `ui-ux-design-reference` skill for style and palette guidance; if browser research is available and permitted, collect 3–5 references
...
Multi-platform social image design: HTML/CSS → screenshot export. Uses the bundled `ui-ux-design-reference`, `brand-guidelines`, and `design-system` skills; screenshot export runs through Chrome headless, Playwright, or Puppeteer (see the reference).

Load `references/social-photos-design.md` for sizes, templates, best practices.

### Social Photos: Workflow

1. **Orchestrate** — Track the steps below with the runtime's native task list; use parallel subagents for independent work where the harness supports them
2. **Analyze** — Parse prompt: subject, platforms, style, brand context, content elements
3. **Ideate** — 3-5 concepts, present them to the user
4. **Design** — bundled `brand-guidelines` → `design-system` → `ui-ux-design-reference` skills; HTML per idea × size
...
1. **Brand** (brand-guidelines skill) → Define colors, typography, voice
2. **Tokens** (design-system skill) → Create semantic token layers
3. **Implement** (shadcn-tailwind-ui skill) → Configure Tailwind, shadcn/ui
...
**Bundled sub-skills:** brand-guidelines, design-system, shadcn-tailwind-ui
**Related Skills:** ui-ux-design-reference
