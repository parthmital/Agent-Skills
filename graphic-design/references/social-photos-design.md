...
Design social media images via HTML/CSS rendering + screenshot export. Orchestrates the bundled `ui-ux-design-reference`, `brand-guidelines`, and `design-system` skills; screenshot export runs through Chrome headless, Playwright, or Puppeteer.
...
Where the harness supports parallel subagents, use them for independent tasks (e.g., multiple HTML files for different sizes).
...
1. **`brand-guidelines`** — Extract brand colors, fonts, voice from user's project
2. **`design-system`** — Get design tokens (spacing, typography scale, color palette)
3. **`ui-ux-design-reference`** — Layout, hierarchy, visual balance; search a different style, palette, or font pairing per concept for design variety.
